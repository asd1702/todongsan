# API_SPEC_v3.md - Insight-Reputation Service
...
| 섹션 6-1, 6-4 | `AI_GENERATION_FAILED` 확정: Member-Point 담당자가 환불 로직 추가, 이 서비스는 예외 발생만 담당. `[팀 결정 필요]` 제거 |
...
| AI_GENERATION_FAILED | 500 | Claude API 호출 실패. Member-Point 서비스가 환불 처리한다. 이 서비스는 예외만 발생시킨다. |
