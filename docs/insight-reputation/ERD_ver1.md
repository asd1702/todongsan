# docs/insight-reputation/ERD_ver1.md

> Insight-Reputation Service의 데이터베이스 설계 문서  
> v1: 방문 인증 쿨다운 30일 수정, 거주지역 변경 쿨다운 컬럼 추가

---

## 변경 내역 (ERD.md → ERD_ver1.md)

| 테이블 | 변경 내용 |
|---|---|
| `reputation` | `residence_changed_at` 컬럼 추가 (거주지역 변경 30일 쿨다운 기준) |
| `visit_certification` | UNIQUE 제약 변경 (일일 1회 → 지역당 1건 유지), `last_certified_at` 컬럼 추가 (30일 쿨다운 Service 레이어 처리) |
...
CREATE TABLE visit_certification (
    ...
    certified_at        DATETIME        NOT NULL,           -- 최초 인증 시점
    created_at          DATETIME        NOT NULL,
    updated_at          DATETIME        NOT NULL,
    PRIMARY KEY (id),
...
- 변경 성공 시 `residence_changed_at` 업데이트, `activity_confirmed_at` 초기화
